...
RSpec.describe 'Create CustomerSubscription', type: :request do
  before :each do
    ...
    #confirm subscription exists, then create customer subscription
    ...
    expect(customer_subscription.first.keys).to match_array [:id, :name, :status, :price, :frequency_delivered, :created_at, :updated_at]
  end
end
