...
RSpec.describe 'Tea Index', type: :request do
  before :each do
    Tea.delete_all
  ...
  it 'It can show all Teas', :vcr do
    ...
    expect(teas[:data].first[:attributes].keys).to match_array [:name, :description, :image, :keywords, :origin, :api_id, :brew_time, :temperature]
  end
end
